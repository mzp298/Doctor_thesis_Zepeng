...
\begin{figure}[h!]
	...
	\label{kinhard}
\end{figure}
Kinematic hardening rules are necessary, especially for the case of unloading and cyclic loading.In Kinematic Hardening the current loading surface is assumed not to expand but to move as a rigid body within the stress space (\figref{kinhard}(b)). The use
...
In uniaxial cyclic loading, there will be 3 kinds of loading regimes, as is shown in \figref{backstress}:
...
In all cases, we get by integrating Eq.\eqref{localisation}, Eq.\eqref{kinematichardening} with the use of Eq.\eqref{plasticflow}.
\begin{equation}
\left( \uline{\uline{S}}-\uline{\uline{b}}\right) (s,t+dt)=\dfrac{\left( \uline{\uline{S}}-\uline{\uline{b}}\right)_{trial} (s,t+dt)}{1+\eta},
\end{equation}

and because of the yield condition Eq.\eqref{yieldlimit}, we have
...
\begin{tabular}{l|l}
\hline
Full numerical strategy: & $1000$ time steps $\times$ $1024$ scales $\times$  $1\times10^{6}$ cycles       \\ \hline
Optimal cyclic strategy: & \begin{tabular}[l]{@{}l@{}} $1000$ time steps $\times$ $1024$ scales $\times$  $5$ cycles until stabilization  \\$+$ $1\times10^{6}$ cycles $\times$ ($1000$ time steps $\times$ reduction factor)      \end{tabular}  \\ \hline
Ratio between optimal and full: & $\approx \dfrac{reduction \; factor}{1024 \; scales}=\dfrac{1}{11703}$       \\ \hline
\end{tabular}
\end{table}
...
\begin{itemize}
	...
	\item repeated integration of damage through the different sequences using Eq.\eqref{eq.optimal}.
